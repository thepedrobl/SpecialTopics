...
disp("Unst mf transfer time = " + un_tt*t_ref/365/86400 + " yr")
disp("Stab mf transfer time = " + -st_tt*t_ref/365/86400 + " yr")
%%
figure(231)
    clf
...
figure(232)
    ...
    plot3(mf_st.X(min_ix,1), mf_st.X(min_ix,2), mf_st.X(min_ix,3),...
        '^g','MarkerSize',15)
    % projections
    projections(mf_un, mf_st)
    axis equal
    grid on
    view(az,el)
    xlabel('x')
    ylabel('y')
    zlabel('z')
    legend("L3 unstable manifold", "L1 stable manifold", ...
        "Sun", "L3 (initial)", "L1 (target)", "Manifold transfer")
